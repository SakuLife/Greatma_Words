...
async def generate_token():
    """Generate YouTube token for CI/CD use."""
    print("\n" + "=" * 70)
    print("YouTube Token Generator")
    print("=" * 70)
    print("\nThis script will:")
    print("1. Open browser for Google OAuth authentication")
    print("2. Generate token.json with valid refresh token")
    ...
    if not os.path.exists(settings.youtube_client_secrets_file):
        print(f"\n❌ ERROR: {settings.youtube_client_secrets_file} not found")
        print("\nPlease follow these steps:")
        print("1. Go to Google Cloud Console")
        print("2. Enable YouTube Data API v3")
        print("3. Create OAuth 2.0 Client (Desktop App)")
    ...
    print("\n" + "-" * 70)
    print("Step 1: Authenticating with YouTube API")
    print("-" * 70)
    print("\nBrowser will open for OAuth consent...")
    print("Please:")
    print("  1. Select your Google account")
    print("  2. Click 'Allow' to grant YouTube permissions")
    print("  3. Close browser when you see 'authentication successful'")

    try:
        # Authenticate
        youtube = YouTubeAnalytics()
        await youtube.authenticate(token_file=token_file)
        ...
        try:
            ...
            channel_info = youtube.service.channels().list(
            ...
            if channel_info.get("items"):
                ...
                print(f"✅ Token works! Connected to channel: {channel_title}")
                print(f"   Subscribers: {subscriber_count}")
            else:
                print("⚠️  Warning: No channel found for this account")
        ...
        print("3. Name: YOUTUBE_TOKEN_JSON_B64")
        ...
        output_file = "youtube_token_b64.txt"
        ...
        print("2. Update GitHub Secret: YOUTUBE_TOKEN_JSON_B64")
